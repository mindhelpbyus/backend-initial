...
function getDynamoDBClient() {
    if (!doc) {
        // Configure DynamoDB client with local endpoint if specified
        const dynamoConfig = { region: REGION };
        ...
        const ddb = new client_dynamodb_1.DynamoDBClient(dynamoConfig);
        doc = lib_dynamodb_1.DynamoDBDocumentClient.from(ddb);
    }
    return doc;
}
...
function verifyAuth(event) {
    ...
    if (!match)
        return null;
    try {
        return jsonwebtoken_1.default.verify(match[1], JWT_SECRET);
    }
    catch (e) {
        return null;
    }
...
const handler = async (event) => {
    ...
    try {
        ...
        if (path === "/auth/signup" && method === "POST") {
            ...
            const token = jsonwebtoken_1.default.sign({ patientId, email, role: "patient" }, JWT_SECRET, { expiresIn: "7d" });
        ...
        if (path === "/auth/login" && method === "POST") {
            ...
            if (!ok)
                return jsonResponse(401, { message: "invalid credentials" });
            const token = jsonwebtoken_1.default.sign({ patientId: user.patientId, email: user.email, role: user.role || "patient" }, JWT_SECRET, { expiresIn: "7d" });
